...
def translate(text, source_lang, target_lang):
    ...
    Translate ONLY the sentence below from {AYA_LANG[source_lang]} to {AYA_LANG[target_lang]} without changing tone, prosody, expression of sentence.
    ...
    try:
        ...
        print(f"Translation error (HTTP {e.response.status_code}): {e.response.text}")
        print(f"Ensure Ollama is running and the model '{GEMMA_MODEL}' is loaded.")
        return None
    ...
    for lang in XTTS_LANG.keys():
        ...
        try:
            wav = tts.tts(
                ...
                repetition_penalty=12.5,   # Increased to prevent robotic loops
                ...
                max_ref_len=15,            # Consistent with app.py for uniform prosody
                enable_text_splitting=False # Set to False to avoid KeyError with Hindi
            )
